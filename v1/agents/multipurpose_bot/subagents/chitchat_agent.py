...
class ChitchatAgent(BaseAgent):
    ...
    def __init__(self, **kwargs):
        ...
        super().__init__(
            name="ChitchatAgent", system_prompt=self._get_chitchat_prompt(), **kwargs
        )
    ...
    def generate_conversational_response(
        ...
        return {
            "metadata": {**state.get("metadata", {}), "base_response": response.content}
        }
    ...
    def add_personality_touches(
        ...
        if conversation_depth > 2 and len(base_response) > 50:
            ...
            enhanced = self.model.invoke(
                [
                    SystemMessage(
                        content=personality_prompt.format(response=base_response)
                    )
                ]
            )
        ...
        return {
            ...
            "metadata": {**state.get("metadata", {}), "enhanced": True},
        }
...
class PersonalityChitchatAgent(ChitchatAgent):
    ...
    def _get_chitchat_prompt(self) -> str:
        ...
        base_prompt = personalities.get(
            self.personality_type, personalities["friendly"]
        )
